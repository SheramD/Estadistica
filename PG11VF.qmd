...
**Examinar cómo varía la autopercepción del impacto de las redes sociales sobre la salud mental y el rendimiento académico según el sexo y el ciclo académico.**

VARIABLES INVOLUCRADAS: sexo , ciclo , concentracion_rendimiento_academico , impacto_redes_salud_mental

# Histograma ciclo
hist(DF_1$ciclo, main="Distribución de Ciclo Académico", xlab="Ciclo", col="lightblue", breaks=10)

library(ggplot2)
library(dplyr)
library(scales)

# Filtrado para evitar NA
DF_1_filtrado <- DF_1 %>%
  filter(!is.na(impacto_redes_salud_mental), !is.na(sexo))

# Convertir ordinal a factor con etiquetas
DF_1_filtrado$impacto_redes_salud_mental <- factor(DF_1_filtrado$impacto_redes_salud_mental,
                                                    levels = c("1","2","3","4","5"),
                                                    labels = c("1 = Poco", "2", "3", "4", "5 = Mucho"))

# Gráfico lado a lado para comparar percepciones entre sexos
ggplot(DF_1_filtrado, aes(x = impacto_redes_salud_mental, fill = sexo)) +
  geom_bar(position = position_dodge()) +
  labs(title = "Comparación de percepción del impacto en salud mental según sexo",
       x = "Nivel de impacto",
...
# Filtrar datos para evitar NA
DF_1_filtrado <- DF_1 %>%
  filter(!is.na(afecta_concentracion_rendimiento), !is.na(sexo))

# Convertir variable ordinal a factor con etiquetas claras
DF_1_filtrado$afecta_concentracion_rendimiento <- factor(DF_1_filtrado$afecta_concentracion_rendimiento,
                                                        levels = c("1","2","3","4","5"),
                                                        labels = c("1 = Poco", "2", "3", "4", "5 = Mucho"))

# Gráfico de barras lado a lado para comparación por sexo
ggplot(DF_1_filtrado, aes(x = afecta_concentracion_rendimiento, fill = sexo)) +
  geom_bar(position = position_dodge()) +
  labs(title = "Comparación de percepción del impacto en rendimiento académico según sexo",
       x = "Nivel de impacto",
       y = "Cantidad de encuestados",
       fill = "Sexo") +
  theme_minimal() +
...
# En esta grafica se nos da a entender que los varones tienden percibir a las redes sociales con un mayor impacto en su rendimiento académico,


# Tabla de frecuencias de sexo
sexo_freq <- table(DF_1$sexo)
...
pie(sexo_freq,
    labels = sexo_labels,
    main = paste0("Distribución de Sexo (Total: ", total_obs, " observaciones)"),
    col = c("skyblue", "pink"))
...
# Primero, filtramos para eliminar NA en las variables ordinales
DF_1_filtrado <- DF_1 %>%
  filter(!is.na(impacto_redes_salud_mental),
         !is.na(afecta_concentracion_rendimiento))
...
# Aqui nos indica que tenemos encuestados de todos los ciclos y una mayor densidad en los ciclos 5 y 7 en el caso de mujeres y 3 y 6 en el caso de varones,
...
ggplot(DF_1_filtrado, aes(x = sexo, fill = factor(impacto_redes_salud_mental))) +
  ...
  scale_fill_manual(values = paleta_colores,
                    name = "Impacto en salud mental",
                    labels = etiquetas_impacto) +
  scale_y_continuous(labels = scales::percent_format()) +
  labs(title = "Impacto percibido en salud mental según sexo",
       x = "Sexo",
       y = "Proporción") +
  theme_minimal()

#Podemos observar que la proporción de la percepción del impacto de las redes sociales en la salud mental es similar entre hombres y mujeres, lo que indica que 
 # esta percepción no varía significativamente según el sexo del encuestado. Sin embargo, se destaca que, en general, los participantes tienden a percibir un # # impacto moderado a alto, ubicándose mayormente en niveles 3 y 4 de la escala de percepción.

# Gráfico Impacto en rendimiento académico vs Sexo
ggplot(DF_1_filtrado, aes(x = sexo, fill = factor(afecta_concentracion_rendimiento))) +
  geom_bar(position = "fill") +
  scale_fill_manual(values = paleta_colores,
                    name = "Impacto en rendimiento académico",
                    labels = etiquetas_impacto) +
  scale_y_continuous(labels = scales::percent_format()) +
  labs(title = "Impacto percibido en rendimiento académico según sexo",
       x = "Sexo",
       y = "Proporción") +
  theme_minimal()
